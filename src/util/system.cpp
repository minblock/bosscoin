...
const char * const BITCOIN_CONF_FILENAME = "bosscoins.conf";
...
#else
    const char* pszModule = "bosscoins";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bosscoins";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bosscoins";
#else
    // Unix
    return pathRet / ".bosscoins";
#endif
#endif
}
